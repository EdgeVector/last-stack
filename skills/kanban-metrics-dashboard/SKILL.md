...
description: |
  Build or refresh an ad-hoc metrics/benchmark view over kanban or LastDB
  ...
  of the same dashboard in brain before rebuilding from scratch — this
...
1. `brain_search` for the metric you're about to build (e.g. "kanban
...
2. If no prior record exists, you're building the first version — plan to
   create the brain reference record in step 4 so the *next* request finds
   it.
...
- **Velocity / throughput**: commit-count-on-main per day per repo
  ...
  (fold, brain, kanban, exemem-infra, exemem-workspace, last-stack, lastgit
  is a reasonable default set) — NOT `kanban list` completion timestamps.
- **Board-state snapshot** (funnel by column, WIP, backlog size): `kanban
...
Write (or update) a brain `reference` record, tagged with the relevant
subsystem (e.g. `kanban`, `lastdb`), recording:
...
brain record's body/timestamp in place — don't create a second reference
