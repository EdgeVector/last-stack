...
3. Prefer narrow reads for routine automation. For an interactive audit, a full
   `fkanban list --all --json` snapshot is acceptable if the node is responsive.
...
```bash
fkanban list --all --json > /tmp/fkanban-board.json

# Active missing dependency slugs
jq -r '.[] | select(.column!="done" and ((.missingDeps//[])|length>0))
  | [.slug,.column,((.missingDeps//[])|join(","))] | @tsv' /tmp/fkanban-board.json
...
jq -r '.[] | select(.column!="done" and .block_status=="needs_human"
  ...
  | [.slug,.column,.repo,.base,.kind,.block_reason] | @tsv' /tmp/fkanban-board.json

# Pickup-ready count
jq '[.[] | select(.column=="todo" and (.blocked|not)
  and (.kind=="pr" or .kind=="validation")
  and ((.repo//"")!="") and ((.base//"")!=""))] | length' /tmp/fkanban-board.json
```
