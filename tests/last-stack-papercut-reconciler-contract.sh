...
printf '{"column":"done"}\n'
SH
chmod +x "$fake_bin/brain" "$fake_bin/kanban"

export TEST_APPEND_LOG="$tmp/appends.log"
export LAST_STACK_PAPERCUT_LIFECYCLE_BRAIN="$fake_bin/brain"
export LAST_STACK_PAPERCUT_LIFECYCLE_KANBAN="$fake_bin/kanban"

out="$(PATH="/usr/bin:/bin" "$helper" --limit 5 --json)"
printf '%s\n' "$out" | jq -e '.ok == true and .fixed == 1 and .scanned == 1' >/dev/null
