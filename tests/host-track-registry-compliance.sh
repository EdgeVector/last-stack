...
# Default registry seeds: lastgit/lastdb/lastdbd exempt; last-stack/remote +
# flipped agent CLIs (brain/situations/kanban/fkanban) artifact; remaining
# local-safe CLIs non_compliant until producers migrate (expected red).
...
printf '%s\n' "$default_report" | jq -e '
  ...
  and any(.apps[]; .app == "lastsecrets" and .registry_compliance == "non_compliant")
  and any(.apps[]; .app == "configurations" and .registry_compliance == "non_compliant")
