...
grep -q 'Ready for pickup promotion' "$program_driver"
grep -q 'means `Kind: pr`' "$program_driver"
grep -q 'If the next frontier is terminal proof state' "$program_driver"
grep -q 'rewriting it as pickup work' "$program_driver"
grep -q 'Existing terminal,' "$program_driver"
...
grep -q 'pending valid ones to `backlog`' "$program_driver"
grep -q 'Promote only a `Kind: pr` terminal harness to default' "$program_driver"
...
grep -q '`board-grooming-latest` is a rolling' "$groom_board"
grep -q '`brain put --allow-shrink`' "$groom_board"
grep -q 'Never append to this record' "$groom_board"

north_star_hygiene="$ROOT/routines/north-star-hygiene.md"
grep -q 'Do not stock pickup `todo`' "$north_star_hygiene"
grep -q 'never files or promotes' "$north_star_hygiene"
grep -q '`Kind: pr` harness' "$north_star_hygiene"
...
grep -q -- '--pr-url "lastgit://' "$pickup"
grep -q 'in-flight-deploy-pending' "$pickup"
grep -q 'awaiting-deploy' "$pickup"
...
grep -q 'pending valid non-PR `DONE-WHEN`' "$pickup"
grep -q 'Only `Kind: pr` child frontiers are pickup work' "$pickup"
grep -q 'feature-prove` / `kanban-watch` own the proof evaluation path' "$pickup"
...
grep -q 'command -v <cmd>' "$pickup"
grep -q 'Routines and Loom do not' "$pickup"
...
grep -q 'Bootstrap the isolated worktree' "$agent"
grep -q 'do not assume a fresh worktree has' "$agent"
grep -q 'bun install --frozen-lockfile' "$agent"
grep -q 'Do not accept a lockfile rewrite' "$agent"
...
grep -q 'command -v <cmd>' "$watch"
grep -q 'Routines and Loom do not' "$watch"
...
grep -q 'pending non-PR `DONE-WHEN`' "$watch"
