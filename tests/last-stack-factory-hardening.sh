...
bash -n "$ROOT/bin/last-stack-canary-loom"
...
[ ! -f "$ROOT/routines/lastdb-canary-red-heal.md" ] || fail "canary-red-heal prompt still present (removed by the release loop)"
[ ! -f "$ROOT/routines/lastdb-canary-promote-prepare.md" ] || fail "canary-promote-prepare prompt still present (removed by the release loop)"
[ -f "$ROOT/routines/lastdb-canary-candidate-set.md" ] || fail "canary-candidate-set prompt missing"
grep -q 'last-stack-canary-candidate-gate' "$ROOT/routines/lastdb-canary-candidate-set.md" \
  || fail "canary-candidate-set prompt missing its gate"
